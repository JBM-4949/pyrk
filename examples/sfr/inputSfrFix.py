...
t_cool = units.Quantity(520.0, units.degC).to(units.kelvin)
t_inlet = units.Quantity(510.0, units.degC).to(units.kelvin)
...
nu = 0.047 * (1 - math.exp(-3.8 * (pitch_to_diam - 1))) * \
    (pow(pe, 0.77) + 250.0)
h_cool = nu * k_cool / dh
h_cool = 1.0e5 * units.joule / units.second / \
    units.kelvin / pow(units.meter, 2)

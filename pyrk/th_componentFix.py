...
class THComponent(object):
    ...
    def add_conduction(self, env, k=None, thickness=None, area=None, conductance=None, 
                   inner_radius=None, outer_radius=None, length=None, model="plate"):
        '''Add conduction in the self.cond dictionary
        
        :param env: name of the component that heat is transfered to/from
        :type env: str
        :param k: thermal conductivity (if not using a pre-defined model)
        :type k: float with units W/(m*K)
        :param thickness: thickness of the conductive layer
        :type thickness: float with units m
        :param area: heat transfer area
        :type area: float with units m^2
        :param conductance: direct thermal conductance (optional, instead of calculating from geometry)
        :type conductance: float with units W/K
        :param inner_radius: inner radius for cylindrical or spherical geometries
        :type inner_radius: float with units m
        :param outer_radius: outer radius for cylindrical or spherical geometries
        :type outer_radius: float with units m
        :param length: length for cylindrical geometry
        :type length: float with units m
        :param model: conduction model to use ('constant', 'plate', 'cylinder', 'sphere')
        :type model: str
        '''
        if conductance is not None:
            # If a direct conductance value is provided, use the constant model
            cond_model = ConductiveModel(k0=conductance, model="constant")
        elif isinstance(k, ConductiveModel):
            # If a ConductiveModel is directly provided
            cond_model = k
        else:
            # Create a new ConductiveModel with the given parameters
            if model == "constant" and k is not None:
                cond_model = ConductiveModel(k0=k, model="constant")
            elif model == "plate" and all(x is not None for x in [k, thickness, area]):
                cond_model = ConductiveModel(k0=k, thickness=thickness, area=area, model="plate")
            elif model == "cylinder" and all(x is not None for x in [k, inner_radius, outer_radius, length]):
                cond_model = ConductiveModel(k0=k, inner_radius=inner_radius, outer_radius=outer_radius, 
                                             length=length, model="cylinder")
            elif model == "sphere" and all(x is not None for x in [k, inner_radius, outer_radius]):
                cond_model = ConductiveModel(k0=k, inner_radius=inner_radius, outer_radius=outer_radius, 
                                             model="sphere")
            else:
                raise ValueError("Insufficient parameters provided for the selected conduction model")
    
        # Store the conduction relationship in the dictionary
        self.cond[env] = {
            "model": cond_model,
            "area": area
    }
